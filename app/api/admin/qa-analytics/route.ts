...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const [todayResult, weekResult, aggregatesResult, topQuestionsResult] = await Promise.all([
    // Count today
    supabase
      .from('qa_analytics')
  ...
  for (const result of [todayResult, weekResult, aggregatesResult, topQuestionsResult]) {
    if (result.error) {
  ...
  return NextResponse.json({
    ...
    topQuestions: topQuestionsByFrequency(topQuestionsResult.data ?? []),
  });
}
